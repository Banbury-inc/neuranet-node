...
async function connect(username, senderSocket) {
    console.log("receiver function called");
    const file_header = `GREETINGS::::END_OF_HEADER`;
        ...
        if (buffer.includes(endOfHeader)) {
            ...
            if (endOfHeaderIndex !== -1) {
                ...
                if (fileType === 'SMALL_PING_REQUEST') {
                    await src_1.neuranet.handlers.small_ping_request(username, senderSocket);
                }
                if (fileType === 'PING_REQUEST') {
                    await src_1.neuranet.handlers.ping_request(username, senderSocket);
                }
                if (fileType === 'FILE_REQUEST') {
                    await src_1.neuranet.handlers.file_request(senderSocket, file_name, file_size);
                }
